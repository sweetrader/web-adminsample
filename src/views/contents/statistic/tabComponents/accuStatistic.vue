<template>
  <div class="app-container">
    ...
    <div v-if="showStatistic" v-loading="dataLoading" class="accu_stat_flex_wrapper">
      <div>
        <div class="accu_statistic_wrapper">
          <!-- 이용 고객 수 -->
          <el-card class="statistic_card_inn_style">
            <div class="statistic_data_contents">
              <div class="data_wrapper">
                <div class="title_text">
                  {{ $t('statistic.monthPointCnt') }}
                </div>
        ...
        <!-- 분석 -->
        <div class="accu_analysis_statistic_wrapper">
          <div class="analysis_wrapper">
            <div style="margin-bottom: 15px;">
              {{ $t('statistic.sixMonthAccuInfo') }}
            </div>
            ...
            <!--
                <el-table :data="week.detailList" border style="min-width: 100%">
                  ...
                  <el-table-column :label="$t('statistic.poingAccuCnt')" align="center">
                    <template slot-scope="scope">
                      {{ scope.row.pitCnt | toThousandFilter }}
                    </template>
                  </el-table-column>
                  <el-table-column :label="$t('statistic.accuPointSum')" align="center">
                    <template slot-scope="scope">
      ...
      <!-- 2번째 통계 추가 -->
      <div class="accu_chart_statistic">
        <!-- 메뉴 -->
        <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleMenuSelect">
          <el-menu-item :index="'1'">{{ $t('statistic.accuCnt') }}</el-menu-item>
          <el-menu-item :index="'2'">{{ $t('statistic.accuPoint') }}</el-menu-item>
        </el-menu>

        <div class="accu_contents_wrapper">
          <div class="chart_wrapper">
            ...
            <div v-if="activeIndex === '1'">
              ...
              <pie-chart
                ...
                :chart-name="$t('statistic.accuCnt')"
            ...
            <div v-if="activeIndex === '2'">
              ...
              <pie-chart
                v-if="$store.getters.roles.length > 0 && $store.getters.roles[0].authority !== 'MCHT'"
                :chart-name="$t('statistic.accuPoint')"
...
export default {
  ...
  data() {
    return {
      ...
      weekInfo: [],
      barChartCtgr: [],
      chartName: this.$t('statistic.accuCnt'),
      chartData: [],
      lineName: this.$t('statistic.accuPoint'),
      lineData: [],
      sixMonthData: [],
      /* 2번째 통계 추가 */
      reload: true,
      ...
      // 제휴사 바 차트
      weekCtgrData: [],
      weekCntName: this.$t('statistic.accuCnt'),
      weekCntData: [],
      weekSumName: this.$t('statistic.accuPoint'),
      weekSumData: []
    }
  },
  created() {
    ...
    chartSetting(stat) {
      ...
      if (stat.last6monthsList) {
        for (let i = 0; i < stat.last6monthsList.length; i++) {
          ...
          this.lineData.push(Number(stat.last6monthsList[i].pitSum))
        }
      }
...
// 기본 정보
.accu_analysis_statistic_wrapper {
  width: 900px;
  margin: 20px 0 32px 0;
  display: flex;
  justify-content: space-between;
...
// 추가 스타일
.accu_stat_flex_wrapper {
  display: flex;
  flex-wrap: wrap;
}

.accu_chart_statistic {
  width: 900px;
  min-width: 900px;
  padding: 20px;
  .accu_contents_wrapper {
    display: flex;
